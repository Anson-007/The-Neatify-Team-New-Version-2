...
export default function Payment({ user }) {
  ...
  const handlePlaceOrderAndPay = async () => {
    ...
    try {
      ...
      // ✅ Sync with Supabase Auth Metadata for the Dashboard
      const cleanPhone = phone.replace(/\D/g, "");
      const formattedPhoneForMeta = cleanPhone.startsWith("91") && cleanPhone.length === 12
        ? `+${cleanPhone}`
      ...
      await supabase.auth.updateUser({
        data: {
          ...
          full_name: firstName,
          phone_number: formattedPhoneForMeta,
        },
      });
              ...
              {selectedServices.map((s, i) => (
                <div key={i} className="service-item">
                  ...
                  <div className="service-item-price-row">
                    <span className="service-item-price">{getCurrency(s.price)}{formatPrice(s.price)}</span>
                    {s.original_price && (
                      <span className="mrp">{getCurrency(s.original_price)}{formatPrice(s.original_price)}</span>
                    )}
            ...
            <div className="summary-bottom">
              ...
              <div className="total-row total-row-final">
                ...
                <div className="total-row-right">
                  <strong>{currency}{finalTotalAmount.toFixed(2)}</strong>
                  {totalOriginalAmount > totalAmount && (
                    <span className="mrp">{currency}{totalOriginalAmount}</span>
                  )}
